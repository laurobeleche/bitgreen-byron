...
- [Development guidelines](#development-guidelines)
    - [General Byron Core](#general-byron-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Byron Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/byron-official/byron/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_byron
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_byron --log_level=test_suite
$ valgrind -v --leak-check=full src/byrond -printtoconsole
```
...
# A coverage report will now be accessible at `./test_byron.coverage/index.html`.
```

**Sanitizers**

Byron Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 27215.
...
Some of these are maintained by active developers of Byron Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Byron Core so that they can be integrated
...
file descriptors it will cause problems with Byron's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Byron's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:byron/byron.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `byron-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `byron-cli` and the GUI debug console use this table to determine how to
